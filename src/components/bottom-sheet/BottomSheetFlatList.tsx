...
import { View, StyleSheet, BackHandler, FlatListProps } from 'react-native'
...
import { BottomSheetProps, BottomSheetRef, BottomSheetScrollProps, MAX_HEIGHT_PERCENTAGE, PADDING, SCREEN_HEIGHT, SPRING_CONFIG } from '.';





const BottomSheetFlatList = forwardRef<BottomSheetRef, BottomSheetScrollProps & FlatListProps<any>>(
    ({ children, height = "70%", data, footer, footerHeight = 0, CellRendererComponent, renderItem, ...rest }, ref) => {
        ...
        const OPEN_HEIGHT = -heightPX - PADDING - footerHeight
        ...
        return (
            <>
                ...
                <GestureDetector gesture={Gesture.Pan().onBegin(() => { })}>
                    <Animated.View style={[styles.container, animatestyle]} className={"dark:bg-secondary bg-white relative"}>
                        ...
                        <GestureDetector gesture={Gesture.Simultaneous(createPanGesture(true), Gesture.Native(),)} >
                            <Animated.FlatList
                                style={{ maxHeight: heightPX - footerHeight }}
                                onScroll={onScroll}
